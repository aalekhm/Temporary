...
void CollisionReport::applyReponse(float CoR)
{
	...
	m_vcoll = (a->m_velocity - b->m_velocity);
	float vn = m_vcoll * m_ncoll;
	
	if(vn > 0.0f) 
		return;

	float numer = -(1.0f + CoR) * vn;
	float denom = (a->m_invmass + b->m_invmass);
	Vector j = m_ncoll * (numer / denom);

	a->m_velocity += j * a->m_invmass;
	b->m_velocity -= j * b->m_invmass;
}