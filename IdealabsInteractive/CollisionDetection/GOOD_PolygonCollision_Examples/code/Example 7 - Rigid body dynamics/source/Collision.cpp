...
ContactPair& ContactPair::operator += (const ContactPair& other)
{
	m_position[0] += other.m_position[0];
	m_position[1] += other.m_position[1];

	Vector d(m_position[0] - m_position[1]);
	m_distanceSquared = (d * d);
	return *this;
}

ContactPair& ContactPair::operator *= (const float k)
{
	m_position[0] *= k;
	m_position[1] *= k;
	m_distanceSquared = (k * k);
	return *this;
}

ContactPair& ContactPair::operator /= (const float k)
{
	const float inverse = 1.0f / k;
	return (*this) *= inverse;
}

ContactPair ContactPair::operator + (const ContactPair& other) const
{
	ContactPair temp(*this);
	temp += other;
	return temp;
}

ContactPair ContactPair::operator * (const float k) const
{
	ContactPair temp(*this);
	temp *= k;
	return temp;
...
ContactPair	ContactManifold::reduction() const
{
	ContactPair singleContact;

	for(int i = 0; i < m_count; i ++)
	{
		singleContact += m_contact[i];
	}
	singleContact /= m_count;
