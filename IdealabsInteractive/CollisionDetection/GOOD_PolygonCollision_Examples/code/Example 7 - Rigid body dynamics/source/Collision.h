...
struct ContactPair
{
	...
	ContactPair& operator *= (const float k);
	ContactPair& operator /= (const float k);

	ContactPair operator + (const ContactPair& other) const;
	ContactPair operator * (const float k) const;
	ContactPair operator / (const float k) const;
};
