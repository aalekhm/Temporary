...
void CReader3DS::makeModelList(t3DModel* pGame3DModel) {
	pGame3DModel->modelList = glGenLists(1);
	
	glNewList(pGame3DModel->modelList, GL_COMPILE);
		glScalef(1/pGame3DModel->reductionFactor, 1/pGame3DModel->reductionFactor, 1/pGame3DModel->reductionFactor);
		draw3DSScene(pGame3DModel);
	glEndList();
}

///*
void CReader3DS::draw3DSScene(t3DModel* pGame3DModel) {
...
//MessageBox(NULL, "adas", "Asdasd", MB_OK);
			...
			*/
			...
			glEnable(GL_TEXTURE_2D);
			glColor3ub(255, 255, 255);
			glBindTexture(GL_TEXTURE_2D, pGame3DModel->MODEL_TEXTURE_ID);
		}
		else {
...
void CReader3DS::computeNormals(t3DModel *pModel) {
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		{												
			...
			vVector1 = getVector(vPoly[0], vPoly[2]);		// Get the vector of the polygon (we just need 2 sides for the normal)
			vVector2 = getVector(vPoly[2], vPoly[1]);		// Get a second vector of the polygon
