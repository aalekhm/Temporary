...
int initGL(GLvoid) {
	...
	glNewList(listName, GL_COMPILE);
		...
		glBegin(GL_TRIANGLES);
			...
			glVertex2f(0.5, -0.5);
		glEnd();
		glTranslatef(1.5, 0.0, 0.0);//If you call transformation commands within a display list, 
									//don't forget to take into account the effect those commands 
									//will have later in your program. 
	glEndList();
...
int drawGLScene() {
	...
	for(int i = 0; i < 10; i++) {
		glTranslatef(-1.0+i*0.5, 0.0, 0.0);
		glCallList(listName);
	}	
