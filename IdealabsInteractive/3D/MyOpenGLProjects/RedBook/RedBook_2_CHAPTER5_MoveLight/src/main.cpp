...
int initGL(GLvoid) {
	/*
	GLfloat matAmbientColor[] = {1.0, 0.0, 0.0, 1.0};
	GLfloat matDiffuse[] = {0.0, 1.0, 1.0, 1.0};
	GLfloat matSpecular[] = {0.0, 1.0, 1.0, 1.0};
	GLfloat matShininess[] = {100.0};

	
	glLightfv(GL_LIGHT0, GL_AMBIENT, matAmbientColor);
...
int drawGLScene() {
	...
	glPushMatrix ();
		...
		glPushMatrix ();
			...
			//RedBook says:
			//If lighting isn't enabled, the current color is simply mapped onto the current vertex, 
			//and no calculations concerning normals, light sources, the lighting model, and material 
			//properties are performed. Here's how to enable lighting:
			//Hence, 
			glDisable (GL_LIGHTING);		//Disable Lightning so that it doesnt affect the WireCube colour appearance
			glColor3f (0.0, 1.0, 1.0);		//Set wire frame Colour
			glutWireCube (0.1);				//and draw it
			glEnable (GL_LIGHTING);			//Now enable lightning for the other objects in the scene...
		glPopMatrix ();
	...
	//float lightPos[] = {cameraPosX, cameraPosY, cameraPosZ, 1.0};
