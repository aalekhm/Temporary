...
bool NeHeLoadBitmap(LPCSTR fileName, GLuint &texID) {
	...
	/*	Texture coordinates can comprise one, two, three, or four coordinates. 
		...
		them from object coordinates (x, y, z, and w) and from evaluator coordinates 
		(u and v)
	*/
...
int initGL(GLvoid) {
	...
	if (!NeHeLoadBitmap("Data/BeachStones.bmp", texture[1]))			// Load The Bitmap
		return FALSE;											// Return False If Loading Failed
...
float xCoeff, yCoeff, zCoeff = 0.0, wCoeff = 0.0;
...
int drawGLScene() {
	...
	glPushMatrix();
		//glRotatef(teaPotRot, 0.0, 1.0, 0.0);	
	...
	//currentCoeff[0] = xCoeff;
	//currentCoeff[1] = yCoeff;
	//currentCoeff[2] = zCoeff;
	//currentCoeff[3] = wCoeff;
