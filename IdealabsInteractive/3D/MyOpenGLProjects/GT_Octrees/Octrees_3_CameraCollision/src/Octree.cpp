...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	if( m_pWorld )
	{
		...
		for(int i = 0; i < m_pWorld->numOfObjects; i++)
		{
			...
			if( m_pWorld->pObjects[i].pFaces )
			{
				delete [] m_pWorld->pObjects[i].pFaces;	
				m_pWorld->pObjects[i].pFaces = NULL;
			}		

			// Free the index data if it's not NULL
			if( m_pWorld->pObjects[i].pIndices )
			{
				delete [] m_pWorld->pObjects[i].pIndices;	
				m_pWorld->pObjects[i].pIndices = NULL;
			}
		}
...
int COctree::getSceneTriangleCount(t3DModel *pWorld) {
	...
	for(int i = 0; i < pWorld->numOfObjects; i++)
	{
		...
		numberOfTriangles += pWorld->pObjects[i].numOfFaces;
	}
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	for(int i = 0; i < pWorld->numOfObjects; i++)
	{
		...
		numberOfVerts += pWorld->pObjects[i].numOfVertices;

		// Add the current object's vertices up
		for(int n = 0; n < pWorld->pObjects[i].numOfVertices; n++)
		{
			// Add the current vertex to the center variable (Using operator overloading)
			m_vCenter = m_vCenter + pWorld->pObjects[i].pVertices[n];
		}
	}
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	for(int i = 0; i < pWorld->numOfObjects; i++)
	{
		...
		for(int j = 0; j < pWorld->pObjects[i].numOfVertices; j++)
		{
			// Get the distance in width, height and depth this vertex is from the center.
			currentWidth  = fabsf(pWorld->pObjects[i].pVertices[j].x - m_vCenter.x);	
			currentHeight = fabsf(pWorld->pObjects[i].pVertices[j].y - m_vCenter.y);		
			currentDepth  = fabsf(pWorld->pObjects[i].pVertices[j].z - m_vCenter.z);
...
void COctree::createNewNode(	t3DModel *pWorld, 
	...
	for(int i = 0; i < pWorld->numOfObjects; i++) {
		...
		pTempWorld->pObjects[i].numOfFaces  = pList[i].totalFaceCount;
		pTempWorld->pObjects[i].materialID  = pObject->materialID;
		pTempWorld->pObjects[i].bHasTexture = pObject->bHasTexture;
		pTempWorld->pObjects[i].pVertices   = pObject->pVertices;

		// Allocate memory for the new face list
		pTempWorld->pObjects[i].pFaces = new tFace [pTempWorld->pObjects[i].numOfFaces];
		...
		for(int j = 0; j < pObject->numOfFaces; j++) {
			...
			if(pList[i].pFaceList[j]) {
				...
				pTempWorld->pObjects[i].pFaces[index] = pObject->pFaces[j];
				index++;
			}
		}
	}
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	for(int i = 0; i < pWorld->numOfObjects; i++)
	{
		...
		if(pTempWorld->pObjects[i].pFaces)
			delete [] pTempWorld->pObjects[i].pFaces;
	}
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
		...
		for(int i = 0; i < pWorld->numOfObjects; i++)
		{
			...
			for(int j = 0; j < pObject->numOfFaces; j++) {
				...
				for(int whichVertex = 0; whichVertex < 3; whichVertex++) {
					...
					CVector3 vPoint = pObject->pVertices[pObject->pFaces[j].vertIndex[whichVertex]];
		...
		for(int i = 0; i < pWorld->numOfObjects; i++)
		{
			...
			for(int j = 0; j < pWorld->pObjects[i].numOfFaces; j++)
			{
...
void COctree::assignTrianglesToNode(t3DModel *pWorld, int numberOfTriangles)
{
	...
	for(int i = 0; i < m_pWorld->numOfObjects; i++)
	{
		...
		m_pWorld->pObjects.push_back(newObject);
		...
		m_pWorld->pObjects[i].numOfFaces = numOfFaces;
		...
		m_pWorld->pObjects[i].pFaces = new tFace [numOfFaces];
		m_pWorld->pObjects[i].pIndices = new UINT [numOfFaces * 3];

		// Initialize the face indices for vertex arrays (are copied below
		memset(m_pWorld->pObjects[i].pIndices, 0, sizeof(UINT) * numOfFaces * 3);

		// Copy the faces from the partition passed in to our end nodes face index list
		memcpy(m_pWorld->pObjects[i].pFaces, pObject->pFaces, sizeof(tFace) * numOfFaces);
		...
		{
			m_pWorld->pObjects[i].pIndices[j]     = m_pWorld->pObjects[i].pFaces[j / 3].vertIndex[0];
			m_pWorld->pObjects[i].pIndices[j + 1] = m_pWorld->pObjects[i].pFaces[j / 3].vertIndex[1];
			m_pWorld->pObjects[i].pIndices[j + 2] = m_pWorld->pObjects[i].pFaces[j / 3].vertIndex[2];
		}
		...
		//Aa - Do not Delete, used for Camera Collisions
		//delete [] m_pWorld->pObjects[i].pFaces;
		//m_pWorld->pObjects[i].pFaces = NULL;
	}
...
/////// * /////////// * /////////// * NEW * /////// * /////////// * /////////// *
	...
	if(!g_Frustum->isCubeInFrustum(	pNode->m_vCenter.x, pNode->m_vCenter.y, 
...
void COctree::createDisplayList(COctree *pNode, t3DModel *pRootWorld, int displayListOffset)
{
	...
	else 
	{
		...
		while(counter < objectCount)
		{
			...
			t3DObject *pObject     = &pNode->m_pWorld->pObjects[i];
			t3DObject *pRootObject = &pRootWorld->pObjects[i];
			...
			if(pRootObject->bHasTexture) 
			{
				...
				glBindTexture(GL_TEXTURE_2D, textures[pRootObject->materialID]);
			} 
			else 
			{
...
char www[255] = {0};
sprintf(www, "%d size pColor = %d %d %d", pRootWorld->pMaterials.size(), pColor[0], pColor[1], pColor[2]);
//MessageBox(NULL, www, "Asasd", MB_OK);
			...
			if(pRootObject->pTexVertices) 
			{
				...
				glTexCoordPointer(2, GL_FLOAT, 0, pRootObject->pTexVertices);
			}

			// Make sure we have vertices to render
			if(pRootObject->pVertices)
			{
				...
				glVertexPointer(3, GL_FLOAT, 0, pRootObject->pVertices);
			}
...
void COctree::listNodeInfo(int ID, int level) {
	char strLog[255] = {0};
	...
	if(m_bSubDivided) {
		...
		for(int i = 0; i < 8; i++) {
			if(m_pOctreeNodes[i]) {
				strLog[0] = 0;
				for(int j = 0; j < nxtLevel; j++)
					strcat(strLog, "\t");
				char log[255] = {0};
				sprintf(log, "%d LEVEL : %d\r\n", i, level);
				strcat(strLog, log);
				//fwrite(strLog, 1, strlen(strLog), gConsoleLogFilePtr);

m_pOctreeNodes[i]->m_bHasCamera = false;

				if(m_pOctreeNodes[i]->isSubDivided()) {
					m_pOctreeNodes[i]->listNodeInfo(i, nxtLevel);
				}
				else {
						CVector3 camPos = gCamera.cameraPosition;
						if(	(camPos.x >= m_pOctreeNodes[i]->getCenter().x-m_pOctreeNodes[i]->getWidth()/2 && camPos.x <= m_pOctreeNodes[i]->getCenter().x+m_pOctreeNodes[i]->getWidth()/2)
							&&
							(camPos.y >= m_pOctreeNodes[i]->getCenter().y-m_pOctreeNodes[i]->getWidth()/2 && camPos.y <= m_pOctreeNodes[i]->getCenter().y+m_pOctreeNodes[i]->getWidth()/2)
							&&
							(camPos.z >= m_pOctreeNodes[i]->getCenter().z-m_pOctreeNodes[i]->getWidth()/2 && camPos.z <= m_pOctreeNodes[i]->getCenter().z+m_pOctreeNodes[i]->getWidth()/2)
						) {
CVector3 LEFT_TOP_BACK = CVector3(	m_pOctreeNodes[i]->getCenter().x - m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().y + m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().z - m_pOctreeNodes[i]->getWidth()/2
								);
CVector3 RIGHT_TOP_BACK = CVector3(	m_pOctreeNodes[i]->getCenter().x + m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().y + m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().z - m_pOctreeNodes[i]->getWidth()/2
								);
CVector3 LEFT_DOWN_BACK = CVector3(	m_pOctreeNodes[i]->getCenter().x - m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().y - m_pOctreeNodes[i]->getWidth()/2,
									m_pOctreeNodes[i]->getCenter().z - m_pOctreeNodes[i]->getWidth()/2
								);
CVector3 RIGHT_DOWN_BACK = CVector3(	m_pOctreeNodes[i]->getCenter().x + m_pOctreeNodes[i]->getWidth()/2,
										m_pOctreeNodes[i]->getCenter().y - m_pOctreeNodes[i]->getWidth()/2,
										m_pOctreeNodes[i]->getCenter().z - m_pOctreeNodes[i]->getWidth()/2
									);
/*
...
glColor3f(255, 0, 0);
glBegin(GL_QUADS);	//Remember, Anti-Clockwise
	glVertex3f(LEFT_DOWN_BACK.x, LEFT_DOWN_BACK.y, LEFT_DOWN_BACK.z);
	glVertex3f(RIGHT_DOWN_BACK.x, RIGHT_DOWN_BACK.y, RIGHT_DOWN_BACK.z);
	glVertex3f(RIGHT_TOP_BACK.x, RIGHT_TOP_BACK.y, RIGHT_TOP_BACK.z);
	glVertex3f(LEFT_TOP_BACK.x, LEFT_TOP_BACK.y, LEFT_TOP_BACK.z);
glEnd();
if(g_bLighting)
	glEnable(GL_LIGHTING);
//*/
m_pOctreeNodes[i]->m_bHasCamera = true;
							sprintf(debugString[3], "CAMERA AT %d %d %d", i, nxtLevel, m_pOctreeNodes[i]->getTriangleCount());
							//sprintf(debugString[4], "%0.2f %0.2f %0.2f__%0.2f %0.2f %0.2f__%0.2f %0.2f %0.2f__%0.2f %0.2f %0.2f",	LEFT_DOWN_BACK.x, LEFT_DOWN_BACK.y, LEFT_DOWN_BACK.z,
							//																										RIGHT_DOWN_BACK.x, RIGHT_DOWN_BACK.y, RIGHT_DOWN_BACK.z, 																									
							//																										RIGHT_TOP_BACK.x, RIGHT_TOP_BACK.y, RIGHT_TOP_BACK.z,
							//																										LEFT_TOP_BACK.x, LEFT_TOP_BACK.y, LEFT_TOP_BACK.z 
							//																									);
sprintf(debugString[4], "ObjectList Size = %d", m_pOctreeNodes[i]->m_pObjectList.size());


//if(gWriteNodeInfoToFile) {
	//char lllog[255] = {0};
	//sprintf(lllog, "-------------------------------------------------------------------------\r\n");
	//fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);


	int MAX_OBJECTS = m_pOctreeNodes[i]->m_pObjectList.size();

	//sprintf(lllog, "\tMAX_OBJECTS = %d, TRIANGLE_COUNT = %d \r\n", MAX_OBJECTS, m_pOctreeNodes[i]->getTriangleCount());
	//fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

	int counter = 0, triangleCount = 0;

	///*
	while(counter < MAX_OBJECTS) {

		int j = m_pOctreeNodes[i]->m_pObjectList[counter];
		
		//sprintf(lllog, "OBJECT ID = %d\r\n", j);
		//fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

		t3DObject* pObject = &m_pOctreeNodes[i]->m_pWorld->pObjects[j];
		t3DObject* rootObject = &gWorldModel.pObjects[j];
		CVector3* pVertices = rootObject->pVertices;
		CVector3* pNormals = rootObject->pNormals;
		
		int numOfFaces = pObject->numOfFaces;

		for(int m = 0; m < numOfFaces; m++) {
			int vertexIdx0 = pObject->pFaces[m].vertIndex[0];
			int vertexIdx1 = pObject->pFaces[m].vertIndex[1];
			int vertexIdx2 = pObject->pFaces[m].vertIndex[2];

/*
			sprintf(lllog, "\tindex = %d %d %d\r\n", vertexIdx0, vertexIdx1, vertexIdx2);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

			sprintf(lllog, "\t\tVERTEX %d = %0.2f %0.2f %0.2f\r\n", vertexIdx0, pVertices[vertexIdx0].x, pVertices[vertexIdx0].y, pVertices[vertexIdx0].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);
			sprintf(lllog, "\t\tNORMAL = %0.2f %0.2f %0.2f\r\n", pNormals[vertexIdx0].x, pNormals[vertexIdx0].y, pNormals[vertexIdx0].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

			sprintf(lllog, "\t\tVERTEX %d = %0.2f %0.2f %0.2f\r\n", vertexIdx1, pVertices[vertexIdx1].x, pVertices[vertexIdx1].y, pVertices[vertexIdx1].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);
			sprintf(lllog, "\t\tNORMAL = %0.2f %0.2f %0.2f\r\n", pNormals[vertexIdx1].x, pNormals[vertexIdx1].y, pNormals[vertexIdx1].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

			sprintf(lllog, "\t\tVERTEX %d = %0.2f %0.2f %0.2f\r\n", vertexIdx2, pVertices[vertexIdx2].x, pVertices[vertexIdx2].y, pVertices[vertexIdx2].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);
			sprintf(lllog, "\t\tNORMAL = %0.2f %0.2f %0.2f\r\n", pNormals[vertexIdx2].x, pNormals[vertexIdx2].y, pNormals[vertexIdx2].z);
			fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);

			triangleCount++;
*/
			CVector3 vPolygon[] = {pVertices[vertexIdx0], pVertices[vertexIdx1], pVertices[vertexIdx2]};
			if(gCamera.checkCameraCollision(vPolygon, 3)) {
					glDisable(GL_LIGHTING);
					glColor3f(255, 0, 0);
					glBegin(GL_TRIANGLES);
						glVertex3f(pVertices[vertexIdx0].x, pVertices[vertexIdx0].y, pVertices[vertexIdx0].z);
						glVertex3f(pVertices[vertexIdx1].x, pVertices[vertexIdx1].y, pVertices[vertexIdx1].z);
						glVertex3f(pVertices[vertexIdx2].x, pVertices[vertexIdx2].y, pVertices[vertexIdx2].z);
					glEnd();
					if(g_bLighting)
						glEnable(GL_LIGHTING);
			}
	...
	//sprintf(lllog, "\tACTUAL TRIANGLE_COUNT = %d, TRIANGLES FOUND = %d\r\n", m_pOctreeNodes[i]->getTriangleCount(), triangleCount);
	//fwrite(lllog, 1, strlen(lllog), gConsoleLogFilePtr);
	//*/
//	gWriteNodeInfoToFile = false;
//}
							//sprintf(strLog, "_____________________________________________CAMERE IS AT %d %d\r\n", i, nxtLevel);
							//fwrite(strLog, 1, strlen(strLog), gConsoleLogFilePtr);
						}
				}
			}
	...
	/*
		...
		MessageBox(NULL, "asdasdas", "Asdasdas", MB_OK);

		CVector3 camPos = gCamera.cameraPosition;
		if(	(camPos.x >= m_vCenter.x-m_Width*2 && camPos.x <= m_vCenter.x+m_Width*2)
			&&
			(camPos.y >= m_vCenter.y-m_Width*2 && camPos.y <= m_vCenter.y+m_Width*2)
			&&
			(camPos.z >= m_vCenter.z-m_Width*2 && camPos.z <= m_vCenter.z+m_Width*2)
		) {
			//MessageBox(NULL, "asdasdas", "Asdasdas", MB_OK);
			sprintf(strLog, "_____________________________________________CAMERE IS HERE\r\n");
			fwrite(strLog, 1, strlen(strLog), gConsoleLogFilePtr);
		}

		sprintf(strLog, "_____________________________________________CAMERE IS NOT HERE\r\n");
		fwrite(strLog, 1, strlen(strLog), gConsoleLogFilePtr);
