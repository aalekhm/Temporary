...
void CFrustum::calculateFrustum() {
	...
	float nearPlaneHeight	= 2 * tan(FOV/2) * nearPlaneDistance;
	float nearPlaneWidth	= nearPlaneHeight * whRatio;

	float farPlaneHeight = 2 * tan(FOV/2) * farPlaneDistance;
	float farPlaneWidth	= farPlaneHeight * whRatio;
	
	
	CVector3 camPos		= gCamera.cameraPosition;
	CVector3 vCamDir	= normalize(gCamera.viewVector);
	CVector3 vCamUp		= normalize(gCamera.upVector);
	CVector3 vCamRight	= normalize(gCamera.rightVector);

	CVector3 farCenter = camPos + (vCamDir * farPlaneDistance);
	CVector3 nearCenter = camPos + (vCamDir * nearPlaneDistance);

	CVector3 farTL = farCenter + (vCamUp * (farPlaneHeight/2)) - (vCamRight * (farPlaneWidth/2));
	CVector3 farTR = farCenter + (vCamUp * (farPlaneHeight/2)) + (vCamRight * (farPlaneWidth/2));
	CVector3 farBL = farCenter - (vCamUp * (farPlaneHeight/2)) - (vCamRight * (farPlaneWidth/2));
	CVector3 farBR = farCenter - (vCamUp * (farPlaneHeight/2)) + (vCamRight * (farPlaneWidth/2));

	CVector3 nearTL	= nearCenter + (vCamUp * (nearPlaneHeight/2)) - (vCamRight * (nearPlaneWidth/2));
	CVector3 nearTR	= nearCenter + (vCamUp * (nearPlaneHeight/2)) + (vCamRight * (nearPlaneWidth/2));
	CVector3 nearBL	= nearCenter - (vCamUp * (nearPlaneHeight/2)) - (vCamRight * (nearPlaneWidth/2));
	CVector3 nearBR	= nearCenter - (vCamUp * (nearPlaneHeight/2)) + (vCamRight * (nearPlaneWidth/2));

	CVector3 vPlanePoints[] = {farTR, nearTR, nearBR, farBR};
	CVector3 planeNormal;

	////////////////////////////// RIGHT PLANE
	vPlanePoints[3] = farBR;
	vPlanePoints[2] = nearBR;
	vPlanePoints[1] = nearTR;
	vPlanePoints[0] = farTR;
	planeNormal = normal(vPlanePoints);

	mFrustum[RIGHT][X] = planeNormal.x;
	mFrustum[RIGHT][Y] = planeNormal.y;
	mFrustum[RIGHT][Z] = planeNormal.z;
	mFrustum[RIGHT][D] = planeDistance(planeNormal, vPlanePoints[0]);

	////////////////////////////// LEFT PLANE
	vPlanePoints[3] = nearBL;
	vPlanePoints[2] = farBL;
	vPlanePoints[1] = farTL;
	vPlanePoints[0] = nearTL;
	planeNormal = normal(vPlanePoints);

	mFrustum[LEFT][X] = planeNormal.x;
	mFrustum[LEFT][Y] = planeNormal.y;
	mFrustum[LEFT][Z] = planeNormal.z;
	mFrustum[LEFT][D] = planeDistance(planeNormal, vPlanePoints[0]);

	////////////////////////////// TOP PLANE
	vPlanePoints[3] = farTL;
	vPlanePoints[2] = farTR;
	vPlanePoints[1] = nearTR;
	vPlanePoints[0] = nearTL;
	planeNormal = normal(vPlanePoints);

	mFrustum[TOP][X] = planeNormal.x;
	mFrustum[TOP][Y] = planeNormal.y;
	mFrustum[TOP][Z] = planeNormal.z;
	mFrustum[TOP][D] = planeDistance(planeNormal, vPlanePoints[0]);

	////////////////////////////// BOTTOM PLANE
	vPlanePoints[3] = nearBL;
	vPlanePoints[2] = nearBR;
	vPlanePoints[1] = farBR;
	vPlanePoints[0] = farBL;
	planeNormal = normal(vPlanePoints);

	mFrustum[BOTTOM][X] = planeNormal.x;
	mFrustum[BOTTOM][Y] = planeNormal.y;
	mFrustum[BOTTOM][Z] = planeNormal.z;
	mFrustum[BOTTOM][D] = planeDistance(planeNormal, vPlanePoints[0]);

	////////////////////////////// FRONT PLANE
	vPlanePoints[3] = farBL;
	vPlanePoints[2] = farBR;
	vPlanePoints[1] = farTR;
	vPlanePoints[0] = farTL;
	planeNormal = normal(vPlanePoints);

	mFrustum[FRONT][X] = planeNormal.x;
	mFrustum[FRONT][Y] = planeNormal.y;
	mFrustum[FRONT][Z] = planeNormal.z;
	mFrustum[FRONT][D] = planeDistance(planeNormal, vPlanePoints[0]);

	////////////////////////////// BACK PLANE
	vPlanePoints[3] = nearBR;
	vPlanePoints[2] = nearBL;
	vPlanePoints[1] = nearTL;
	vPlanePoints[0] = nearTR;
	planeNormal = normal(vPlanePoints);

	mFrustum[BACK][X] = planeNormal.x;
	mFrustum[BACK][Y] = planeNormal.y;
	mFrustum[BACK][Z] = planeNormal.z;
	mFrustum[BACK][D] = planeDistance(planeNormal, vPlanePoints[0]);
...
bool CFrustum::isCubeInFrustum(float x, float y, float z, float size) {
	...
	for(int i = 0; i < 6; i++ )	{
		...
		if(mFrustum[i][X] * (x - size) + mFrustum[i][Y] * (y - size) + mFrustum[i][Z] * (z - size) + mFrustum[i][D] > 0)
		   continue;
		if(mFrustum[i][X] * (x + size) + mFrustum[i][Y] * (y - size) + mFrustum[i][Z] * (z - size) + mFrustum[i][D] > 0)
		   continue;
		if(mFrustum[i][X] * (x - size) + mFrustum[i][Y] * (y + size) + mFrustum[i][Z] * (z - size) + mFrustum[i][D] > 0)
