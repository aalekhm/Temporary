...
void CLoad3DS::ProcessNextChunk(t3DModel *pModel, tChunk *pPreviousChunk)
{
	...
	while (pPreviousChunk->bytesRead < pPreviousChunk->length)
	{
		...
		case OBJECT:							// This holds the name of the object being read
			...
			m_CurrentChunk->bytesRead += GetString(pModel->pObjects[pModel->numOfObjects - 1].strName);
			
			// Now proceed to read in the rest of the object information
			ProcessNextObjectChunk(pModel, &(pModel->pObjects[pModel->numOfObjects - 1]), m_CurrentChunk);
			break;
...
void CLoad3DS::ReadUVCoordinates(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	pPreviousChunk->bytesRead += fread(&pObject->numTexVertices, 1, 2, m_FilePointer);

	// Allocate memory to hold the UV coordinates
	pObject->pTexVertices = new tVector2 [pObject->numTexVertices];

	// Read in the texture coodinates (an array 2 float)
	pPreviousChunk->bytesRead += fread(pObject->pTexVertices, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);
}
...
void CLoad3DS::ReadVertices(t3DObject *pObject, tChunk *pPreviousChunk)
{
	...
	pPreviousChunk->bytesRead += fread(&(pObject->numOfVertices), 1, 2, m_FilePointer);
	...
	pPreviousChunk->bytesRead += fread(pObject->pVertices, 1, pPreviousChunk->length - pPreviousChunk->bytesRead, m_FilePointer);
...
void CLoad3DS::ComputeNormals(t3DModel *pModel)
{
	...
	for(int index = 0; index < pModel->numOfObjects; index++)
	{
		...
		{												
			...
			vPoly[0] = pObject->pVertices[pObject->pFaces[i].vertIndex[0]];
			vPoly[1] = pObject->pVertices[pObject->pFaces[i].vertIndex[1]];
			vPoly[2] = pObject->pVertices[pObject->pFaces[i].vertIndex[2]];
