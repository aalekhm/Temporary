...
bool		twinkle;					// Twinkling Stars ( NEW )

bool		tPressed = false;			// 'T' pressed ( NEW )

const GLuint num = 50;					// Num of Stars to draw ( NEW )

typedef struct {						// Create A Structure For Star
	int r, g, b;						// Stars Color
	GLfloat distance;					// Stars Distance From Center
		...
		if(PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			if(mActive) {
				if(mKeys[VK_ESCAPE])
					...
					if(mKeys['T'] && !tPressed) {			// Is T Being Pressed And Is tp FALSE
						tPressed = true;					// If So, Make tp TRUE
					...
					if(mKeys[VK_UP]) {				// Is Up Arrow Being Pressed
						tilt-=0.5f;				// Tilt The Screen Up
					}

					if(mKeys[VK_DOWN]) {				// Is Down Arrow Being Pressed
						tilt+=0.5f;				// Tilt The Screen Down
					}

					if (mKeys[VK_PRIOR]) {				// Is Page Up Being Pressed
						zoom-=0.2f;				// Zoom Out
					}

					if (mKeys[VK_NEXT]) {				// Is Page Down Being Pressed
						zoom+=0.2f;				// Zoom In
					}
...
int initGL(GLvoid) {
	...
	if (!NeHeLoadBitmap("Data/Star.bmp", texture[0]))			// Load The Bitmap
	...
	for(loop = 0; loop < num; loop++) {		// Create A Loop That Goes Through All The Stars
		...
		star[loop].distance = ((float)loop/num)*5.0f;	// Calculate Distance From The Center
...
int drawGLScene() {
	...
	for(loop = 0; loop < num; loop++) {					// Loop Through All The Stars
		...
		if (star[loop].distance < 0.0f)			// Is The Star In The Middle Yet
		{
