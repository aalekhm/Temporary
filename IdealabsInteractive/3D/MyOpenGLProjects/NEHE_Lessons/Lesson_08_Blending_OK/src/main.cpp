...
bool		blend = false;				// Blending ON/OFF ( NEW )
bool		lPressed = false;			// 'L' pressed 
bool		fPressed = false;			// 'F' pressed 
bool		bPressed = false;			// 'B' pressed ( NEW )
		...
		if(PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			if(mActive) {
				if(mKeys[VK_ESCAPE])
					...
					if (mKeys['B'] && !bPressed) {		// Is B Key Pressed And bp FALSE?
						bPressed = TRUE;				// If So, bp Becomes TRUE
					...
					if (!mKeys['B'])					// Has B Key Been Released?
					{
						bPressed = FALSE;				// If So, bp Becomes FALSE
					}
