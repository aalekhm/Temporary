...
void checkKeyboardMovement() {
	...
	if(mKeys['W']) {
		cameraPosX -= sin( cameraRotY * piOver180 ) * 2*MOVE_SPEED;
		cameraPosZ -= cos( cameraRotY * piOver180 ) * 2*MOVE_SPEED;
	}
	if(mKeys['S']) {
	...
	if(mKeys['A']) {
		cameraPosX += sin( (cameraRotY-90) * piOver180 ) * 2*MOVE_SPEED;
		cameraPosZ += cos( (cameraRotY-90) * piOver180 ) * 2*MOVE_SPEED;
	}
	if(mKeys['D']) {
...
bool bitmapLoader(LPCSTR fileName, GLuint &textureID) {
	...
	if(!hBMP)
		return false;
MessageBox(NULL, fileName, "0", MB_OK);
...
char ww[255] = "data/";
strcat(ww, pGame3DModel.pMaterials[i].textureFileName);
//sprintf(ww, "Material = %s", pGame3DModel.pMaterials[i].textureFileName);

		// Check to see if there is a file name to load in this material
		if(strlen(pGame3DModel.pMaterials[i].textureFileName) > 0) {
...
replaceJpgToBmp(&ww[0]);
			bitmapLoader(ww, texture[i]);
		}
