...
bool RandomAccessFile::openForRead(const char *fileName) {//, const char *mode) {
	int fileStatus = _access(fileName, 0);
	if(fileStatus == -1) {
		printf("RandomAccessFile:: %s File/Directory does not exist!!!", fileName);
		return false;
	}

	m_pFileName = new CString(fileName);

	fopen_s(&fileObj, m_pFileName->c_str(), "rb");//Read As BINARY
	filePointer = 0;
...
bool RandomAccessFile::openForWrite(const char *fileName) {
	...
	fopen_s(&fileObj, m_pFileName->c_str(), "wb+");//Write as BINARY
	filePointer = 0;
...
const char* RandomAccessFile::openForAppend(const char *fileName) {
	...
	fopen_s(&fileObj, m_pFileName->c_str(), "a+");
...
const char* RandomAccessFile::readCharS() {
	if(fileObj != NULL) {
		unsigned char byte0;
		char* pBuffer = new char[sizeof(char)+1];//allocMemory(sizeof(char));
		pBuffer[sizeof(char)] = '\0';

		fseek(fileObj, filePointer, SEEK_SET);
...
const char* RandomAccessFile::readShortS() {
	if(fileObj != NULL) {
		short LSB;
		char* pBuffer = new char[sizeof(short)+1];//allocMemory(sizeof(short));
...
const char* RandomAccessFile::readIntS() {
	if(fileObj != NULL) {
		char* pBuffer = new char[sizeof(int)+1];//allocMemory(sizeof(int));
...
const char* RandomAccessFile::readLongS() {
	if(fileObj != NULL) {
		char* pBuffer = new char[sizeof(long long)+1];//allocMemory(sizeof(long long));
...
const char *RandomAccessFile::readLine() {
	...
	if(iLineLength > 0) {
		char* line = new char[iLineLength+1];
		::memset(line, 0, sizeof(line));
		line[iLineLength] = '\0';

		fseek(fileObj, prevFilePointer, SEEK_SET);
		
		int iRead = fread(line, sizeof(char), iLineLength, fileObj);
		fseek(fileObj, filePointer, SEEK_SET);
		return line;
	}
...
void RandomAccessFile::write(char* buf, int offset, int len) {
	...
	fwrite(writeFrom, sizeof(char), len, fileObj);
	filePointer += len;
}
...
void RandomAccessFile::writeChars(const char* buf) {
	if(fileObj != NULL) {
		...
		fwrite(buf, sizeof(char), strlen(buf), fileObj);
...
void RandomAccessFile::writeLong(const char* longValue) {
	...
	if(fileObj != NULL) {
		...
		fputc((num0>>0)&0xff, fileObj);
		filePointer += 1;
		fputc((num0>>8)&0xff, fileObj);
		filePointer += 1;
		fputc((num0>>16)&0xff, fileObj);
		filePointer += 1;
		fputc((num0>>24)&0xff, fileObj);
		filePointer += 1;
		//*/
		...
		//*/
		///*
		fputc((num1>>0)&0xff, fileObj);
		filePointer += 1;
		fputc((num1>>8)&0xff, fileObj);
		filePointer += 1;
		fputc((num1>>16)&0xff, fileObj);
		filePointer += 1;
		fputc((num1>>24)&0xff, fileObj);
		filePointer += 1;
		//*/
	}
}
...
//MessageBox(NULL, sLine, "read() --- > bytesToRead", MB_OK);
	...
	while(!isEOF()) {
		if(	len > 0 
			...
			int bytesRead = fread(buf + offset, sizeof(char), bytesToRead, fileObj);
...
void RandomAccessFile::skipBytes(unsigned long long numOfBytesToSkip) {
	if(fileObj != NULL) {
		...
		fseek(fileObj, numOfBytesToSkip, SEEK_SET);
		
		//filePointer = ftell(fileObj);
	}
}
...
long RandomAccessFile::getFileLength() {
	if(fileObj!= NULL) {
		int errNo = fseek(fileObj, 0l, SEEK_END);
//MB(getStringValue(errNo));
		if(errNo == 0) {
			
			//long fileLen = ftell(fileObj);
			//return fileLen;
			
			fpos_t pos_t;
			int err = fgetpos(fileObj, &pos_t);
...
unsigned long long	RandomAccessFile::length() {
	...
	if(fileObj != NULL) {
		long lastPos = ftell(fileObj);//save last position.

		fseek(fileObj, 0l, SEEK_END);
		fileLength = ftell(fileObj);

		fseek(fileObj, lastPos, SEEK_SET);
	}
...
const char *RandomAccessFile::lengthS() {
	...
	char* pBuffer = new char[sizeof(fileLength)+1];//allocMemory(sizeof(fileLength));
