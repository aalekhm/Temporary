...
class CHILD {
	public:
		CHILD() {
			posOffsets = new OFFSETS();
			skinOffsets = new OFFSETS();
			align = new ALIGN();
			parentSize = new WIDGETSIZE();
		};
...
struct SCoordTex
{
	struct SCoordFloat3 coord[4];
	struct SCoordFloat2 tex[4];	
	u32	color;
	u32 zoom;
	u32 angle;	
	u32 flag;			// BIT0.à mixer avec la 3d  BIT1.couche au dessus de la 3d
	u8	blend_disable;	// TRUE = pas de blend / FALSE = blend
	u8	page_tex;		// page de texture (pas plus de 256pages de 256x256x2 autrement ca dépasse 24Mo)
};

struct SSpriteInfo {
	struct SCoordTex	*coord;
	s16					*idx;
	s32					qtelement_old;
	s32					qtelement;	
};
