...
bool		m_bFULLSCREEN = false;		// Fullscreen Flag Set To FALSE By Default
...
int winMainLoop() {
	...
	m_pEngineManager = EngineManager::getInstance();
	{
		int iWidth = 1024;
		int iHeight = 768;
		m_pEngineManager->onSetViewport(iWidth, iHeight);
		m_pEngineManager->setViewport(iWidth, iHeight);
	}
	
	{
		if (!createGLWindow(	"Dream3D!!!", 
								m_pEngineManager->getWidth(), 
								m_pEngineManager->getHeight(), 
								16, m_bFULLSCREEN))
			return 0;
