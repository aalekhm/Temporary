...
namespace cocostudio
{
    ...
    {
		//check if the name contains "_extended". If yes then its an extended scroll view.
		std::string search_str("_extended");
		auto options = (ScrollViewOptions*)scrollViewOptions;
		auto widgetOptions = (WidgetOptions*)options->widgetOptions();
		std::string name = widgetOptions->name()->c_str();

		ScrollView* scrollView=NULL;
		if (name.find(search_str) != std::string::npos)
		{
			scrollView = UIScrollViewExtended::createExtendedScrollView();
		}
		else
		{
