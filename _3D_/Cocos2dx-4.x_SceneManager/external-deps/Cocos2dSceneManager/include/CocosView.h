...
class CocosView
{
	public:
		...
		void									SetCreatorReader(ICocosReader* pCreatorReader);
		...
		const std::string&						GetDesignFileName() const { return m_sDesignFilename; }
		...
		creator::AnimationManager*				GetAnimationManager() { return m_pAnimationManager; }
	...
	protected:
		...
		creator::AnimationManager*				m_pAnimationManager;

		std::string								m_sViewName;

		std::string								m_sAnimIN;
		std::string								m_sAnimOUT;
		std::string								m_sDesignFilename;
	...
	private:
		...
		bool									m_bIsCreatorDesignedScene;

		void									HandleCustomUIParameters(cocos2d::Node* pObject);
		//std::map<std::string, JsonDomObject*>	m_mapCustomUINodeParameters_NameObject;
};