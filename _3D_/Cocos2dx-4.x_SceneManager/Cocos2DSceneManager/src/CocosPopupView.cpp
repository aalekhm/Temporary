...
void CocosPopupView::StartINAnimation(bool IsOpenExplicit)
{
	if (m_pViewNode != NULL && !m_pViewNode->isVisible())
	{
		...
		CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimIN, CocosPopupView::OnINAnimationStart, CocosPopupView::OnINAnimationEnd, this, true, 0);
	}
}
...
void CocosPopupView::StartOUTAnimation(bool IsOpenExplicit)
{
	if (m_pViewNode != NULL && m_pViewNode->isVisible())
	{
		CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimOUT, CocosPopupView::OnOUTAnimationStart, CocosPopupView::OnOUTAnimationEnd, this, true, 0);
	}
}
