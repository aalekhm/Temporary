...
#define HAS_EXTENSION(__fileExtension__, __fileName__) std::string(__fileExtension__) == CocosSceneUtils::GetExtentionName(__fileName__)
...
CocosView::CocosView()
	...
	, m_bIsCreatorDesignedScene(false)
{
...
void CocosView::RefreshDataFromJson(nlohmann::json& jView)
{
	...
	TRY_GET_JSONVAL(m_sDesignFilename,			jView,	std::string, "designFilename", "");
...
void CocosView::SetCreatorReader(ICocosReader* pCreatorReader)
{
	if (pCreatorReader != nullptr)
	{
		m_pCreatorReader = pCreatorReader;
	}
}
...
{                
	...
	if (m_pViewNode == NULL)
	{
		...
		if (NOT m_sDesignFilename.empty())
		{
			if (HAS_EXTENSION("csb", m_sDesignFilename))
			{
				m_pViewNode = cocos2d::CSLoader::createNode(m_sDesignFilename, CC_CALLBACK_1(CocosView::OnNodeLoaded, this));
				
			}
			else
			if (HAS_EXTENSION("ccreator", m_sDesignFilename.c_str()))
			{
				m_bIsCreatorDesignedScene = true;
				m_pViewNode = m_pCreatorReader->createWithFilename(m_sDesignFilename);
			}

			OnNodeLoaded(m_pViewNode);
		}
		...
		if (bCacheFully && m_pViewNode != NULL)
		{
			...
			if (m_bIsCreatorDesignedScene)
			{
				GET_CHILD_BY_NAME(m_pViewNode, m_pAnimationManager, creator::AnimationManager, "creator_animation_manager");
			}
			else
			{
				CREATE_ACTION_AND_RETAIN(m_sDesignFilename, m_pActionTimeline)
			}
...
void CocosView::HandleCustomUIParameters(cocos2d::Node* pObject)
{
	...
	//	{
	//		JsonDomObject* pJsonDomObject = m_mapCustomUINodeParameters_NameObject[sNodeName];
	//
...
void CocosView::StartINAnimation(bool IsOpenExplicit)
{
	if (m_pViewNode != NULL && !m_pViewNode->isVisible())
	{
		...
		if (m_bIsCreatorDesignedScene)
		{
			if (IsOpenExplicit)
			{
				CALL_COCOS_ANIMATION_BY_ACTION(m_pViewNode, m_pAnimationManager, m_sAnimIN, CocosView::OnINAnimationStart, CocosView::OnINAnimationEnd, this, true, 0);
			}
			else
			{
				CALL_COCOS_ANIMATION_BY_ACTION(m_pViewNode, m_pAnimationManager, m_sAnimIN, CocosSceneManager::OnINAnimationStart, CocosSceneManager::OnINAnimationEnd, &CocosSceneManager::Get(), true, 0);
			}
		}
		else
		{
			if (IsOpenExplicit)
			{
				CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimIN, CocosView::OnINAnimationStart, CocosView::OnINAnimationEnd, this, true, 0);
			}
			else
			{
				CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimIN, CocosSceneManager::OnINAnimationStart, CocosSceneManager::OnINAnimationEnd, &CocosSceneManager::Get(), true, 0);
			}
		}
	}
}
...
void CocosView::HandleBG()
{
	...
	if (m_bHasBG)
	{	
		pCocosSceneManager.OpenViewExplicitly(pCocosSceneManager.GetBackgroundView());
	}
	else
	{
...
void CocosView::StartOUTAnimation(bool IsOpenExplicit)
{
	if (m_pViewNode != NULL && m_pViewNode->isVisible())
	{
		if (m_bIsCreatorDesignedScene)
		{
			if (IsOpenExplicit)
			{
				CALL_COCOS_ANIMATION_BY_ACTION(m_pViewNode, m_pAnimationManager, m_sAnimOUT, CocosView::OnOUTAnimationStart, CocosView::OnOUTAnimationEnd, this, true, 0);
			}
			else
			{
				CALL_COCOS_ANIMATION_BY_ACTION(m_pViewNode, m_pAnimationManager, m_sAnimOUT, CocosSceneManager::OnOUTAnimationStart, CocosSceneManager::OnOUTAnimationEnd, &CocosSceneManager::Get(), true, 0);
			}
		}
		else
		{
			if (IsOpenExplicit)
			{
				CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimOUT, CocosView::OnOUTAnimationStart, CocosView::OnOUTAnimationEnd, this, true, 0);
			}
			else
			{
				CALL_COCOS_ANIMATION_BY_ACTION_CSB(m_pViewNode, m_pActionTimeline, m_sAnimOUT, CocosSceneManager::OnOUTAnimationStart, CocosSceneManager::OnOUTAnimationEnd, &CocosSceneManager::Get(), true, 0);
			}
		}
	}
}
...
void CocosView::OnUnLoad()
{
	if (m_bIsNodeReadyForRender)
    {
		...
		if (m_bIsCreatorDesignedScene)
		{
			STOP_ACTION_AND_RELEASE(m_pAnimationManager)
		}
		else
		{
			STOP_ACTION_AND_RELEASE_CSB(m_pActionTimeline)
		}
    }
	CleanUp();
}
