...
class CocosSceneManager : public Singleton<CocosSceneManager>, public IGameMessageHandler
{
	enum STATE_LOADING
	{
		eState_LoadingWait,
	...
	public:
		...
		void									RegisterWithCocosView(std::string sViewName, ICocosReader* pCreatorReader);
		...
		virtual void							HandleMessage(std::string, const MessageData* pMessageData);
		...
		void									SetBackgroundView(std::string sViewName);
		std::string								GetBackgroundView();
	protected:
