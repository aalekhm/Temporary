...
{
	setName("creator_animation_manager");
}
...
{
	AnimationManager::playAnimationClip(target, animationClipName, nullptr, nullptr);
}

void AnimationManager::playAnimationClip(cocos2d::Node *target, const std::string &animationClipName, std::function<void(std::string)> animStartCallback, std::function<void(std::string)> animEndCallback)
{
...
void AnimationManager::playAnimationClip(const std::string &animationClipName, std::function<void(std::string)> animStartCallback, std::function<void(std::string)> animEndCallback)
{
    bool foundTarget = false;
    bool foundAnimationClip = false;
    ...
    for (auto& animationInfo : _animations)
    {
        for (auto& animClip : animationInfo.clips)
        {
            if (animClip->getName() == animationClipName)
            {
				_onAnimationStartListener = animStartCallback;
				_onAnimationEndListener = animEndCallback;
...
void AnimationManager::runAnimationClip(cocos2d::Node *target, AnimationClip* animationClip)
{
	...
	if (_onAnimationStartListener != nullptr)
	{
		_onAnimationStartListener(animationClip->getName());
		_onAnimationStartListener = nullptr;
	}
...
void AnimationManager::removeAnimateClip(cocos2d::Node *target, const std::string &animationClipName)
{
    for (auto iter = _cachedAnimates.begin(), end = _cachedAnimates.end(); iter != end; ++iter)
    {
        ...
        {
			if (_onAnimationEndListener != nullptr)
			{
				_onAnimationEndListener(animationClipName);
				_onAnimationEndListener = nullptr;
			}
...
bool AnimationManager::IsAnimationInfoExists(const std::string& animationClipName)
{
	for (AnimationInfo& pAnimationInfo : _animations)
	{
		for (AnimationClip* pAnimationClip : pAnimationInfo.clips)
		{
			if (pAnimationClip->getName() == animationClipName)
				return true;
		}
	}
