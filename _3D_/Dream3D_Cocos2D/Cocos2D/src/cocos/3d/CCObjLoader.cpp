...
#define TINYOBJ_SSCANF_BUFFER_SIZE  (4096)
    ...
    std::string MaterialFileReader::operator()(const std::string &matId,
        ...
        std::istringstream matIStream(cocos2d::FileUtils::getInstance()->getStringFromFile(filepath).c_str());
        std::string err = LoadMtl(matMap, materials, matIStream);
        if (!matIStream) {
            std::stringstream ss;
            ss << "WARN: Material file [ " << filepath.c_str() << " ] not found. Created a default material.";
            //err.sprintf("%s",ss.str().c_str());
        }
        return err.c_str();
    }
    ...
    std::string LoadObj(std::vector<shape_t> &shapes,
        ...
        std::istringstream ifs(cocos2d::FileUtils::getInstance()->getStringFromFile(filename).c_str());
        if (!ifs) {
